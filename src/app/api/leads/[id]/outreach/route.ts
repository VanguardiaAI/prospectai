...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === "generate_email") {
    ...
    const generated = await generateEmail(lead.name, lead.category, lead.city, lead.website, analysis, tone, fromName, undefined, withStrategyDirective(campaign?.strategy), detectCountryFromPhone(lead.phone) || undefined, campaign?.agencyProfileId ?? undefined);
  ...
  if (action === "generate_wa") {
    ...
    const generated = await generateWhatsApp(lead.name, lead.category, lead.city, lead.website, waAnalysis, tone, fromName, undefined, withStrategyDirective(campaign?.strategy), detectCountryFromPhone(lead.phone) || undefined, campaign?.agencyProfileId ?? undefined);
